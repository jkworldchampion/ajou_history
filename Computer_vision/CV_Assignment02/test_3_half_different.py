...
def main():
    ...
    ##### Set threshold
    threshold_left = 50  # 왼쪽 부분의 세세한 변화를 덜 감지하도록 낮은 threshold 설정
    threshold_right = 30  # 오른쪽 부분의 기본 threshold 설정
    ...
    for image_idx in range(len(input_img)):
        ...
        if background_img is not None:
            ...
            ##### 이미지의 왼쪽과 오른쪽을 나눠 서로 다른 threshold 적용
            h, w = frame_current_gray.shape
            left_diff_abs = diff_abs[:, :w // 2]  # 왼쪽 절반
            right_diff_abs = diff_abs[:, w // 2:]  # 오른쪽 절반

            ##### 왼쪽 부분: 낮은 threshold 적용
            frame_diff_left = np.where(left_diff_abs > threshold_left, 1.0, 0.0)

            ##### 오른쪽 부분: 기본 threshold 적용
            frame_diff_right = np.where(right_diff_abs > threshold_right, 1.0, 0.0)

            ##### 왼쪽과 오른쪽을 다시 합침
            frame_diff = np.hstack((frame_diff_left, frame_diff_right))
